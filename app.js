...
function parseAndSaveDataToIndexedDB(port, parser) {
  return new Promise((resolve, reject) => {
    // Слухаємо дані з серійного порту
    port.on("data", (data) => {
      try {
        // Парсимо дані
        const parsedData = decodeDataFromController(data);

        // Визначаємо, в який магазин записати дані (наприклад, "addresses" чи "orders")
        const storeName = determineStoreName(parsedData); // Ця функція має визначити, в який магазин IndexedDB записати дані

        // Формуємо запит для IndexedDB
        const req = {
          store_name: storeName,
          index_name: "id", // Приклад: індекс по "id", або інший індекс
          request: parsedData, // Дані, які потрібно зберегти
        };

        // Використовуємо метод збереження в IndexedDB
        data_base.save_data(req);

        resolve(parsedData); // Успішний результат, повертаємо розпарсені дані
      } catch (error) {
...
// Функція для парсингу даних із серійного контролера
function decodeDataFromController(data) {
  const textDecoder = new TextDecoder();
  const decodedString = textDecoder.decode(data);

  // Логіка розпарсування рядка в об'єкт
  const parsedData = JSON.parse(decodedString);

  return parsedData;
}

// Функція для визначення магазину збереження (адреса чи замовлення)
function determineStoreName(parsedData) {
  if (parsedData.hasOwnProperty("article")) {
    return "addresses";
  } else if (parsedData.hasOwnProperty("order_id")) {
    return "orders";
  }
  // Додайте інші умови для різних магазинів за потреби
  return "defaultStore";
}
