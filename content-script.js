...
function main() {
	...
	let hendlers = {
		...
		show_barcodes: function () {
			...
			let barcodes_wrapper = this.parentNode.parentNode.parentNode;
					...
					children: [
						...
						},
						{
							...
							hendler: async function (event) {
								let stilage_wrapper = document.querySelector(".select_barcode_stilage");
								if (stilage_wrapper) {
								...
								let stilages_list = Array.from(stilages.querySelectorAll("table tr"));
								stilages_list.shift();
									...
									children: [
										{
											...
											hendler: async function (event) {
												let stilage = await load.fetch({
													url: url.stilage,
													method: "POST",
													body: { id: data[zone_name].stilages_list[event.target.value].id }
												});

												let stilage_row = Array.from(stilage.querySelectorAll("tr"));
												stilage_row.forEach(function (key, index) {
													index = index + 1;
													if (!data[zone_name].stilages_list[event.target.value].row) {
														data[zone_name].stilages_list[event.target.value][index] = {};
													}
													...
													data[zone_name].stilages_list[event.target.value][index] = td.map(function (item) {
														...
														let goods_list = item.title.split('\r').map(function (str) {
															...
															let desc;
															try {
																article = str.match(new RegExp(regExp.article))[1];
																desc = str.replace(regExp.article, '').trim();
															}
															catch (error) {
																alert(error);
																alert(`помилка при отриманні артикулу ${str}`);
																article = false;
																desc = false;
															}
															if (!article) {
