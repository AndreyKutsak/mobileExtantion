...
`;
	...
	const data_base = {
						...
						if (result.length > 0) {
							if (store_item === "addresses") {
								result.forEach(function (res_item) {
									console.log(res_item.article, "article")
		...
		update_data: function (req) {
			...
			const request = store.put(req.request);
			request.onsuccess = function (event) {
				console.log(event.target.result, "success update");
			};
			request.onerror = function (event) {
				console.log(event);
			};
	...
	function main() {
		...
		let hendlers = {
			...
			getToProduction: function () {
				if (
					data_base.data.production.some((obj) => obj.id === this.dataset.id)
				) {
				...
				load
					...
					.then((result) => {
						if (result.body.textContent === "ok") {
							...
							data.delay = delay;
							data_base.data.elaborations[
								`${get.date().year}.${get.date().month}.${get.date().day}.${get.date().hours
		...
		let generate = {
								...
								children: [
									...
									generateSelect(Object.keys(categorys), "change", function () {
										...
										const subCategorySelect = generateSelect(
											...
											function () {
												...
												if (articles_list.length > 0) {
													if (
														num_category_wrpper.querySelector(
															".articles_wrapper"
														)
													) {
														num_category_wrpper
										...
										if (existingSubCategoryWrapper) {
											existingSubCategoryWrapper.replaceWith(
												subCategoryWrapper
											);
										} else {
										...
										},
										{
											...
											children: [
												...
												},
												{
													...
													children: Object.values(item.components).map(
														(part) => {
															return {
																el: "div",
			...
			order: function (data) {
				...
				return generate.message(
					"Сталася помилка під час отримання інформації."
				);
			},
			compare: function () {
				contentWraper.innerHTML = "";
										...
										children: [
											{
												...
												text: `${data_base.data.listArray[item].addingDate.day
													}.${data_base.data.listArray[item].addingDate.month}.${data_base.data.listArray[item].addingDate.year
													}  ${data_base.data.listArray[item].addingDate.hours}:${data_base.data.listArray[item].addingDate.minutes
						...
						children: [
							...
							},
							{
								...
								children: data.map((item) => {
									...
									let save_area =
										data_base.data.addresses[item].save_area_count;
		...
		let load = {
			...
			elaborations: async function (data) {
				...
				const rows = Array.from(
					elaborations.querySelectorAll("table tbody tr")
				);
					...
					rows.map(async (data) => {
						...
						result.forEach((item, index) => {
							if (
								...
								elaborationData.goodId = get.article(
